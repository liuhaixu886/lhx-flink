 ...
 *
 * @author 刘海旭
 * @date 2026/05/26 15:52
 */
public class ods_log_sink {

    /** Kafka 集群地址。 */
    private static final String KAFKA_BOOTSTRAP_SERVERS = "node101:9092,node102:9092,node103:9092";

    /** 原始日志数据主题。 */
    private static final String SOURCE_TOPIC_LOG = "topic_log";

    /** ODS 层日志数据落地主题。 */
    private static final String SINK_TOPIC_ODS_LOG = "ods_log";

    /** ODS 层统一脏数据主题。 */
    private static final String SINK_TOPIC_DIRTY_DATA = "dirty_data";

    /** 当前作业的 Kafka 消费者组。 */
    private static final String CONSUMER_GROUP_ID = "ods_log_sink";
    ...
    public static void main(String[] args) throws Exception {
        // 创建 Flink 流处理环境，当前日志转存作业先使用单并行度便于本地观察输出。
        ...
        // 没有历史消费位点时，从 topic_log 的最早数据开始消费；已有位点时按消费者组位点继续。
        kafkaConsumer.setStartFromGroupOffsets();

        // 实时读取 topic_log 中的原始日志字符串。
        DataStreamSource<String> topicLogStream = env.addSource(kafkaConsumer, "kafka-topic-log-source");

        // 打印读取到的日志，方便开发阶段确认日志格式和流量。
        topicLogStream.print("topic_log");

        // ODS 层只做 JSON 格式合法性校验，不做字段清洗、业务过滤等 DWD 层工作。
        SingleOutputStreamOperator<String> validJsonStream = topicLogStream.process(
        ...
        // 从侧输出流中取出非法 JSON，统一写入 dirty_data，便于后续排查问题日志。
        ...
        // 合法 JSON 写入 ods_log，作为后续 DWD 日志清洗和分流的入口。
        ...
        ).name("kafka-ods-log-sink");

        // 非法 JSON 或空字符串写入 dirty_data，不污染 ods_log。
        ...
        env.execute("ods_log_sink");
    }

    /**
     * 判断消息是否为合法 JSON 对象。
     * ODS 层只判断格式是否能解析，不判断日志字段是否完整或含义是否正确。
     */
